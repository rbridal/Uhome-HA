...
class UhomeDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        # threshold is reached (or a device reports offline). Auth failures set
        # the counter to the threshold immediately because HA stops rescheduling
        # after ConfigEntryAuthFailed.
        self.consecutive_update_failures = 0
        _LOGGER.info(
            "Uhome data coordinator initialized (poll=%ds, discovery=%ds, "
            "availability_threshold=%d)",
            scan_interval,
            discovery_interval,
    ...
    def max_update_failures(self) -> int:
        ...
        return max(
            MIN_MAX_UPDATE_FAILURES,
            min(MAX_MAX_UPDATE_FAILURES, int(raw)),
        )
    ...
    def poll_healthy_enough(self) -> bool:
        ...
        Used by entities instead of ``last_update_success`` so a run of transient
    ...
    def _record_auth_failure(self, err: BaseException) -> None:
        """Set counter to threshold so entities go unavailable immediately."""
        self.consecutive_update_failures = self.max_update_failures
        _LOGGER.error(
            "Uhome auth failure — entities marked unavailable "
            "(threshold=%d): %s",
            self.max_update_failures,
            err,
        )
